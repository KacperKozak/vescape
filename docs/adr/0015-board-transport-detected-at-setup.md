...
- Re-probing clears the stored Board Link when it starts and replaces it only if probing succeeds. Because a Board Session owns a single BLE connection, re-probing a connected Board tears down the live session first.
