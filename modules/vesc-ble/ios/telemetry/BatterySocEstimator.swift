...
/// has run.
///
/// @parity /modules/vesc-ble/android/src/main/java/expo/modules/vescble/telemetry/BatterySocEstimator.kt
...
internal final class BatterySocEstimator {
  ...
  /// Board `batteryConfig` (canonical keys — see `VescBleModule.normalizeBatteryConfig`).
  ...
  /// Locate `cell-presets.json`, whether it landed as a CocoaPods resource bundle (`VescBleAssets`)
  ...
  private static func bundledPresetsJson() -> String? {
    ...
    let candidates: [URL?] = [
      ...
      moduleBundle.url(forResource: "VescBleAssets", withExtension: "bundle").flatMap {
