 ...
 * returns the decoded [ConfigSafetyValues] the config-safety detector evaluates. Nothing is
...
internal object ConfigReplayHarness {
  /**
   * Decoded config-safety values from the recording's config read, or null when the recording holds
   * no completable config exchange (the config-scoped detector then evaluates nothing).
   */
  fun decodeSafetyValues(jsonl: String): ConfigSafetyValues? {
    val reassembler = VescPacketReassembler()
    var state: ConfigRWState = ConfigRWState.Idle
    var captured: ConfigSafetyValues? = null
    ...
    fun dispatch(event: ConfigRWEvent) {
      ...
      for (effect in effects) {
        ...
        // result matters here.
        if (effect is ConfigRWEffect.EmitReadComplete) effect.safety?.let { captured = it }
      }
    }
