...
import { compareMarketingVersions, selectReleaseNotes } from './releaseNotes'

describe('release-note selection', () => {
  test('shows installed and older versions newest-first', () => {
    const notes = [
      { version: '0.82.0', markdown: 'old' },
  ...
  test('selects notes when the runtime does not provide Array.toSorted', () => {
    ...
    try {
      expect(selectReleaseNotes([{ version: '1.0.0', markdown: 'Current' }], '1.0.0')).toEqual([
        { version: '1.0.0', markdown: 'Current' },
      ])
    } finally {
