 ...
 * 2. **Update Warning** — recommends an update once per cold launch.
 * 3. **Community Message** — the head of the announcement queue.
 *
...
export function selectReleaseSurface(inputs: ReleaseSurfaceInputs): ReleaseSurface | null {
  ...
  if (version?.status === 'update-warning' && !inputs.updateWarningDismissed) {
